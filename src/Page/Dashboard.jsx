    ...
    try {
      ...
      const boundingBoxes = detections.map((det, index) => {
        ...
        return {
          ...
          y: normalizedBox ? normalizedBox.width * (detection.image_height || 1) : y1,
        ...
        if (!isStenosis) {
          ...
          const maximumBoxConfidence = Math.max(
            ...boundingBoxes.map((box) => Number(box.confidence ?? 0) * 100)
          )

          if (maximumBoxConfidence < confidenceThreshold) {
            const adjustedThreshold = Math.max(1, Math.floor(maximumBoxConfidence)
            )

            setConfidenceThreshold(adjustedThreshold)

            visualizationNotice =
              `협착 의심 부위 ${boundingBoxes.length}건이 탐지되었습니다. ` + `표시 기준을 ${adjustedThreshold}%로 자동 조정했습니다.`
          } else {
            visualizationNotice =
